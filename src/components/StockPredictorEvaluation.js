...
class StockPredictorEvaluation extends Component {
  constructor(props) {
    super(props);
    this.state = {
      prediction: "Loading...",
  ...
  fetchStockPredictions = () => {
    ...
    fetch(`http://localhost:5000/stock-predictor/evaluate/${this.props.symbol}/${this.state.endDate}/${this.state.previousDays}/${this.state.trainingDays}`)
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({predictionData: responseJson.data, prediction: this.props.symbol.toUpperCase()})
    })
  }
  ...
  renderEvaluationChart = () => this.state.predictionData && (
    <LineChart title={this.props.symbol} width={600} height={300} data={this.state.predictionData}>
      <XAxis dataKey="date"/>
      <YAxis/>
  ...
  render() {
    return(
      <div style={mainContainerStyle}>
        <h1>Prediction Evaluation: {this.state.prediction}</h1>
...
export {StockPredictorEvaluation};